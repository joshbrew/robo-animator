...
class PiRoboConfig(object):
    ...
    def loadConfigs(self, motorCFG_data, animCFG_data):
        ...
        self.motorCFG_data = motorCFG_data
        self.animCFG_data = animCFG_data
    ...
    def run(self):
        ...
        for servo in self.servo_motors:
            ...
            pwm.append(GPIO.PWM(int(servo['GPIO']),freq))
        for io in self.gpio:
...
if __name__ == '__main__':
    # TO RUN LOCAL FILES COMMENT OUT THE SERVER AND UNCOMMENT THE CODE BELOW IT
    
    app.run('0.0.0.0',9000)
    
    # Pass file locations to this with subprocess.Popen(['python','TestRun_server.py',motor.cfg,anim.dat], shell=True)
    # Or os.system('python TestRun_server.py motor.cfg anim.dat')

    #motorCFGPath = sys.argv[2]
    #animCFGPath = sys.argv[3]
    #motorCFG_data = json.load(open(motorCFGPath))
    #animCFG_data = json.load(open(motorCFGPath))
    #cfg = PiRoboConfig()
    #cfg.loadConfigs(motorCFG_data,animCFG_data)
    #cfg.run()